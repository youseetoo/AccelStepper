...
void AccelStepper::disableOutputs()
{
    ...
    if (_enablePin != 0xff)
    {
        ...
        {
            //_externalCallForPin(_enablePin, OUTPUT);
            _externalCallForPin(_enablePin, LOW ^ _enableInverted);
        }
        else
        {
...
void AccelStepper::enableOutputs()
{
    ...
    if (!(_pin[0] & PIN_EXTERNAL_FLAG))
        pinMode(_pin[0], OUTPUT);
    // else if(_externalCallForPin != nullptr)
    //     _externalCallForPin(_pin[0], OUTPUT);
    if (!(_pin[1] & PIN_EXTERNAL_FLAG))
        pinMode(_pin[1], OUTPUT);
    // else if(_externalCallForPin != nullptr)
    //     _externalCallForPin(_pin[1], OUTPUT);
    if (_interface == FULL4WIRE || _interface == HALF4WIRE)
    {
        if (!(_pin[2] & PIN_EXTERNAL_FLAG))
            pinMode(_pin[2], OUTPUT);
        // else if(_externalCallForPin != nullptr)
        //     _externalCallForPin(_pin[2], OUTPUT);
        if (!(_pin[3] & PIN_EXTERNAL_FLAG))
            pinMode(_pin[3], OUTPUT);
        // else if(_externalCallForPin != nullptr)
        //     _externalCallForPin(_pin[3], OUTPUT);
    }
    else if (_interface == FULL3WIRE || _interface == HALF3WIRE)
    {
        if (!(_pin[2] & PIN_EXTERNAL_FLAG))
    ...
    if (_enablePin != 0xff)
    {
        ...
        {
            //_externalCallForPin(_enablePin, OUTPUT);
            _externalCallForPin(_enablePin, HIGH ^ _enableInverted);
        }
        else
        {
            pinMode(_enablePin, OUTPUT);
